...
namespace TCP {
	...
	bool PortForward::AddLocalBasicInfoPool(const std::string& ip, int port)
	{
	...
	bool PortForward::StratPortForward()
	{
		//检查是否有服务器以及本地数据
	...
	SockBingInfo PortForward::InitSockBingInfo(TCPSOCK LID, TCPSOCK SID)
	{
		SockBingInfo info;
		info.LID = LID;
		info.SID = SID;
	...
	bool PortForward::DelSockBingInfo(TCPSOCK LID)
	{
	...
	bool PortForward::DelServeSock(TCPSOCK SID)
	{
	...
	bool PortForward::DelLockSock(TCPSOCK LID)
	{
	...
	SockBingInfo* PortForward::SearchSockBingInfoInLID(TCPSOCK LID)
	{
		for (int i = 0;i < SockBingPool.size();i++) {
			if (SockBingPool.at(i).LID == LID) {
	...
	SockBingInfo* PortForward::SearchSockBingInfoInSID(TCPSOCK SID)
	{
		for (int i = 0;i < SockBingPool.size();i++) {
	...
	ServerBasicInfo PortForward::InitServerBasicInfo(std::string ip, int port)
	{
	...
	LocalPortBasicInfo PortForward::InitLocalBasicInfo(std::string ip, int port)
	{
	...
	ServerBasicInfo* PortForward::GetServerBasicInfo(ServerStrategy type)
	{
		//策略模式 单一策略 延迟优先 数量优先
	...
	void PortForward::forwardData(TCPSOCK fromSock, TCPSOCK toSock, TCP::ForwardType forwardType)
	{
	...
	void PortForward::OnServerConn(TCPSOCK sock)
	{
		//绑定 
	...
	void PortForward::OnServerMessage(TCPSOCK sock, std::string buf)
	{
	...
	void PortForward::OnServerClose(TCPSOCK sock)
	{
	...
	void PortForward::OnClientMessage(TCPSOCK sock, std::string buf)
	{
	...
	void PortForward::OnClientClose(TCPSOCK sock)
	{
